...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $DID = uniqid('D'); // Generate a unique DID
    $name = $_POST['name'];
    ...
    $checkQuery = $conn->prepare("SELECT DID FROM drivers WHERE email = ? OR username = ?");
    ...
    if ($checkQuery->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO drivers (DID, name, address, contact, status, location, email, username, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('sssssssss', $DID, $name, $address, $contact, $status, $location, $email, $username, $password);
